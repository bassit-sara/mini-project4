{{-- หน้าสำหรับ Staff จัดการ (CRUD) รายชื่อ --}}
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('จัดการรายชื่อบุคลากร (Staff)') }}
        </h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead>
                                <tr>
                                    <th class="px-6 py-3 text-left">ชื่อ (User)</th>
                                    <th class="px-6 py-3 text-left">ตำแหน่ง</th>
                                    <th class="px-6 py-3 text-left">ติดต่อ</th>
                                ...
                                @foreach($staffs as $staff)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 text-right">
                                            <a href="{{ route('staff.club-staff.edit', $staff) }}" class="text-indigo-600 hover:text-indigo-900">แก้ไข</a>
                                            <form action="{{ route('staff.club-staff.destroy', $staff) }}" method="POST" class="inline-block ms-2" onsubmit="return confirm('คุณแน่ใจหรือไม่ว่าต้องการลบ?');">
                                                @csrf
                                                @method('DELETE')
