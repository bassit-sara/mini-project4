{{-- หน้าแสดง "รายการจองสถานที่" (ของฉัน) --}}
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('ประวัติการจองสถานที่') }}
        </h2>
    </x-slot>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead>
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left">วัตถุประสงค์</th>
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse($reservations as $reservation)
                                    <tr>
                                        <td class="px-6 py-4">{{ $reservation->place_name }}</td>
                                        <td class="px-6 py-4">{{ $reservation->purpose }}</td>
                                        <td class="px-6 py-4">{{ $reservation->start_date->format('d M Y H:i') }}</td>
                                        <td class="px-6 py-4">
